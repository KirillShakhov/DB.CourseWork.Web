...
function updateCarsTable() {
    ...
    $.ajax({
        ...
        data["list"]['cars'].forEach((i) => {
            ...
            let removeButton = i['creator']['creator_user']['username'] === myname ? "<button class=\"btn-none\" style=\"margin-left: 10px;\" onclick='removeCars(" + i["id_car"] + ")'>\n" +
...
function removeCars(id) {
    $.ajax({
        ...
        if (data["status"] === "ok") {
            tempAlert("Машинка удалена", 3000);
            updateCarsTable();
        } else {
            tempErrorAlert(data["message"], 3000);
        }
